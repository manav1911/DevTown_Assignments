...
const apiKey = `4f9ffa7ce440faaf52b3cfa3fac4b894`
...
const weatherCard = (res) => {
    ...
    const data = `
        <div class="flex justify-center">
            <div class="card min-w-sm max-w-sm border border-gray-100 bg-gray-50 transition-shadow test shadow-lg hover:shadow-shadow-xl w-full bg-blue-600 text-purple-50 rounded-md">
                <h2 class="text-md mb-2 px-4 pt-4">
                <div class="flex justify-between">
                    <div class="badge relative top-0">
                        <span
                            class="mt-2 py-1 h-12px text-md font-semibold w-12px rounded right-1 bottom-1 px-4">${name}
                        </span>
                    </div>
                </div>
                </h2>
                ...
                <div class="flex flex-col justify-center items-center w-96">
                    ...
                    <span>${weather[0].main} ${main.temp_max}°/${main.temp_min}°</span>
                </div>
                <div class="border-l-2 h-20"></div>
                <div class="flex justify-center items-center w-96">
                        <img src="https://openweathermap.org/img/wn/${weather[0].icon}@2x.png" alt="" srcset="" class="h-32 w-32 text-yellow-300"/>
                </div>
                </div>
...
const weatherName = async (location) => {
    await setApiName(location)
    await fetchWeather()
}

const weatherCordinates = async (lat, long) => {
    await setApiCordinates(lat, long)
    await fetchWeather()
}

const findCordinates = () => {
    console.log(navigator.geolocation)
    navigator.geolocation.getCurrentPosition((position) => {
        weatherCordinates(position.coords.latitude, position.coords.longitude)
    })
}
